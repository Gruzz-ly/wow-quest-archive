# Stormcaller Morka (quest)

{{Questbox
 | id = 54694
 | name = Stormcaller Morka
 | category = Nazmir
 | faction = Alliance
 | level = {{level|bfa}}
 | type = Faction Assault Elite World Quest
 | start = Automatic
 | reputation = +150 [[7th Legion (faction)|7th Legion]]
 | experience = 26,800
 | shareable = No
}}

==Objectives==
* [[Stormcaller Morka]] slain

==Description==
:{{text|say|[[Shandris Feathermoon]]|Slaying that target will demoralize the [[Horde]]'s [[soldier]]s. Show them that defeat is inevitable.}}

==Rewards==
You will receive:
* 26,800 [[XP]]
* +150 reputation with [[7th Legion (faction)|7th Legion]]

{{wqr|bfaa}}

==Completion==
:{{text|say|Shandris Feathermoon|Well done! I am glad you are by our side.}}

==Notes==
*Location: [[Gloomwater Span]], [[Nazmir]] during [[Faction Assaults]].

==Patch changes==
*{{Patch 8.1.0|note=Added.}}

==External links==
{{Elinks-quest|54694}}

[[Category:Battle for Azeroth world quests]]
[[Category:Faction Assault quests]]
[[Category:7th Legion quests]]